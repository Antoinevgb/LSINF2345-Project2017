...
%% @private
store_in_buffer(List, Actor) ->
    Members = ?SYNC_BACKEND:membership(),
    Peers = ?SYNC_BACKEND:without_me(Members),

    FillBuff = fun(Peer) ->
        case ets:info(Peer) =:= undefined of
            true ->
                ets:new(Peer, [named_table, public]),
                ets:insert(Peer, {List, Actor});
            false ->
                ets:insert(Peer, {List, Actor})
        end
    end,
    lists:foreach(FillBuff, Peers).
